...
## 6.1) نطاق التاجر — merchant.amialpay.com

نطاق مستقل لبوابة التاجر على **تطبيق Laravel الحالي نفسه**، بلا خدمة ثانية أو قاعدة بيانات منفصلة.
احتفظ بـ `APP_URL` على النطاق الرئيسي، وبنطاق تطبيق Flutter/API كما هو. لا يُفعَّل الفصل قبل نجاح اختبارات الويب.

1. أضف سجل DNS: اسم `merchant` ونوع `A` إلى IP خادم Coolify، أو `CNAME` إلى النطاق الرئيسي إذا كانت إعدادات DNS تسمح.
2. في Coolify أضف `https://merchant.amialpay.com` إلى نطاقات **نفس تطبيق Laravel**، ثم تحقّق من إصدار TLS/HTTPS لهذا الاسم.
3. اترك `SESSION_DOMAIN=` فارغاً و`SESSION_SECURE_COOKIE=true` في الإنتاج. بوابة التاجر تستعمل كوكي جلسة مخصصة ومستقلة.
4. **بعد DNS والشهادة فقط** ضع `AMIAL_MERCHANT_HOST=merchant.amialpay.com` في متغيرات بيئة التطبيق، ثم أعد النشر وافرغ config/route cache بالطريقة المتبعة في النشر.
5. تحقق من `https://merchant.amialpay.com` (يحوّلك إلى `/merchant`) و`https://merchant.amialpay.com/login` (إلى `/merchant/login`). الرابط القديم `https://amialpay.com/merchant/login` يجب أن يحوّل GET فقط قبل إدخال كلمة المرور.
6. جرّب دخول مالك تاجر، المحفظة، الباقات، القطاع، ثم خروج الجلسة. لا تحجب وصول التاجر من التطبيق حتى تنجح هذه المسارات على بيئة تجريبية.

للتراجع الآمن عند تعذّر DNS/TLS: اجعل `AMIAL_MERCHANT_HOST` فارغاً وأعد نشر الإعدادات؛ تعود بوابة `/merchant` على النطاق الرئيسي.
