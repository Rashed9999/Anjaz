...
# ══════════════════════════════════════════════════════════════════════
# AMIAL-GATE-TIMING-001 — **بوّابةٌ لا تقول أين ذهب وقتُها لا تُسرَّع.**
#
# سِتُّ جولاتٍ في جلسةٍ واحدة، كلٌّ نحوَ نصف ساعة. وقيل «الاختباراتُ
# بطيئة» — **وهو تخمينٌ**: لا شيءَ في المخرَج يقيس طبقةً واحدة.
#
# فصار كلُّ رأسِ طبقةٍ يُغلق زمنَ ما قبله، ويُطبع مجموعٌ في الخاتمة.
# ولا كلفةَ له: `SECONDS` مبنيٌّ في الصَدَفة.
# ══════════════════════════════════════════════════════════════════════
_LAYER_NAME=""
_LAYER_T0=0
GATE_T0=$SECONDS
...
_close_layer() {
  [ -z "$_LAYER_NAME" ] && return 0
  local d=$(( SECONDS - _LAYER_T0 ))
  GATE_TIMES="${GATE_TIMES}${d}\t${_LAYER_NAME}\n"
  [ "$d" -ge 20 ] && printf '  \033[2m⏱ %s ث\033[0m\n' "$d"
  return 0
}
...
# يُطبَع في الخاتمة — مرتَّباً بالأثقل، فالنظرةُ الأولى تقع على العنق.
gate_timing_report() {
  _close_layer
  printf '\n\033[1mزمنُ الطبقات (الأثقلُ أوّلاً)\033[0m\n'
  printf "%b" "$GATE_TIMES" | sort -rn | awk -F'\t' '$1>=5 {printf "  %5d ث  %s\n", $1, $2}'
  printf '  ─────────────\n  %5d ث  الإجمالي\n' "$(( SECONDS - GATE_T0 ))"
}
...
# ── ١) تركيب PHP ─────────────────────────────────────────────────────
head_ "١) تركيب PHP"
# **و`-P` لا تُغيّر ما يُفحَص، بل من يفحصه.** قِيست هذه الطبقةُ ٤٢ ث
# على نواةٍ واحدة، و`php -l` عمليّةٌ لكلّ ملفّ — فالتوازي ربحٌ خالص.
# (`-n1` تبقى: `php -l` يقرأ ملفّاً واحداً لا قائمة.)
ERR=$(find app config routes database/migrations tests -name '*.php' -newermt '-14 days' -print0 2>/dev/null \
      | xargs -0 -r -n1 -P "$(nproc 2>/dev/null || echo 4)" php -l 2>&1 \
...
# ── ٤ب) لا إغلاقَ محورٍ بلا دليل ─────────────────────────────────────
#
# **مصفوفةُ المحاور تُكذّب صاحبَها.** سجلُّ الإغلاقات
# (`docs/مصفوفة-المحاور.json`) يُكتب باليد، وكلُّ إغلاقٍ يُشير إلى
# اختباراتٍ تُثبته. وإغلاقٌ يُشير إلى اختبارٍ لا وجودَ له **ادّعاءٌ
# مكتوب** — وهو أخطرُ من عدم الإغلاق: يُقرأ تقدّماً ويُبنى عليه قرارُ
# نشر.
#
# وقد أمسك هذا الحاجزُ **كاتبَه في أوّل تشغيل**: اسمُ دالّةِ اختبارٍ
# كُتب بلا سابقة `test_`، فسقط الادّعاء.
head_ "٤ب) إغلاقاتُ المحاور لها أدلّة"
if [[ -f scripts/axis-matrix.php ]]; then
  AXIS_OUT=$(php scripts/axis-matrix.php --check 2>&1)
  if [[ $? -eq 0 ]]; then
    echo "$AXIS_OUT" | sed 's/^/  /' | sed 's/^  \(  \)/\1/'
    PASS=$((PASS+1))
  else
    echo "$AXIS_OUT" | sed 's/^/  /' | sed 's/^  \(  \)/\1/'
    FAIL=$((FAIL+1))
  ...
  printf '  \033[33m—\033[0m لا مصفوفةَ محاور — تخطّي\n'
fi
...
if [[ -x "$FLUTTER_BIN" && -d "$APP_DIR" ]]; then
  ...
  if [[ -n "$DART_ERR" ]]; then
    ...
    ok "صفر خطأ تصريف — تحليلاً (تحذيرات: $W · ملاحظات: $I)"
    # ══════════════════════════════════════════════════════════════════
    # AMIAL-BUILD-LOCK-001 — **والمحلِّلُ لا يمسّ التبعيّات.**
    #
    # سقط بناءُ Codemagic على `flutter_html` بـ
    # `Method not found: 'matches'` **والبوّابةُ خضراءُ تماماً**:
    # `flutter analyze` يحلّل شيفرتَنا وحدَها، ولا يقرأ سطراً واحداً من
    # `.pub-cache`. فكسرٌ داخل حزمةٍ لا نملكها يمرّ صامتاً.
    #
    # **والتصريفُ الحقيقيُّ في الطبقة العاشرة**: `app_compiles_end_to_end_test`
    # يستورد `main.dart` فيجرّ الشجرةَ كلَّها، و`flutter test` يبني نواةً
    # قبل التشغيل — وهي الخطوةُ نفسُها التي سقطت هناك
    # (`Target kernel_snapshot_program failed`).
    #
    # وأُثبت بالعكس: أُعيد العطلُ بنصّه داخل الحزمة، **فأمسكه الاختبارُ
    # وقال المحلِّلُ «صفر خطأ»**.
    # ══════════════════════════════════════════════════════════════════
  fi
...
if [[ $FAST -eq 0 ]]; then
  ...
  # ══════════════════════════════════════════════════════════════════
  # ══════════════════════════════════════════════════════════════════
  #  AMIAL-GATE-SHARD-001 — **على أربعة أنوية لا واحد.**
  #
  #  قِيس بعد أن صارت البوّابةُ تقيس نفسَها: هذه الطبقةُ **٤٦٦ ث من ٦٢٨**
  #  — أي ثلاثةُ أرباع الوقت على ربع الآلة.
  #
  #  والتقسيمُ أُثبت قبل اعتماده، **والعددُ هو الحَكَم**:
  #
  #      متتابعاً : 466 ث · 3118 ناجحاً · 17 مُقلِقاً · 2 مُخطّىً
  #      بأربعٍ   : 155 ث · 3118 ناجحاً · 17 مُقلِقاً · 2 مُخطّىً
  #
  #  **ونقصٌ في العدد كان سيعني تقسيماً يبتلع لا تسريعاً** — فلا يُعتمد.
  #
  #  والارتدادُ متتابعٌ لا مُخطّىً: إن رفض المقسِّمُ (لا صلاحيّةَ إنشاء
  #  قواعد، أو تقسيمٌ أسقط ملفّاً) يخرج بـ٣، فتجري المجموعةُ كما كانت.
  #  **وسرعةٌ تتحوّل إلى تخطٍّ عند أوّل عائقٍ أسوأ من البطء.**
  # ══════════════════════════════════════════════════════════════════
  OUT=$(bash "$(dirname "${BASH_SOURCE[0]}")/test-shards.sh" 2>&1)
  SHARD_RC=$?

  if [[ $SHARD_RC -eq 3 ]]; then
    printf '  \033[33m—\033[0m %s\n' "التقسيمُ تعذّر — تُشغَّل متتابعةً:"
...
# ── إيصالُ ساهر ──────────────────────────────────────────────────────
#
# SAHER-GATE-003 — يُسجَّل حكمُ هذه الجولة لشجرة المحتوى المفحوصة، فيصير
# «دُفع بلا فحص» اكتشافاً مرئيّاً بدل أن يُكتشف بعد ساعة.
#
# **ولا يُغيَّر رمزُ خروج البوّابة مهما وقع.** §2.1: ساهرٌ راصدٌ لا تابع،
# وأمرُ تسجيلٍ يسقط لأنّ القاعدةَ نائمةٌ يجب ألّا يقلب بوّابةً خضراءَ إلى
# حمراء. **وراصدٌ يُسقط ما يرصده أسوأ من غيابه.** ولذلك `|| true`
# وتحويلُ المخرجات — والأمرُ نفسُه يخرج بالنجاح دائماً.
if [[ $FAIL -eq 0 ]]; then SAHER_VERDICT=PASS; else SAHER_VERDICT=FAIL; fi
if [[ $FAST -eq 1 ]]; then SAHER_FAST=--fast; else SAHER_FAST=; fi

php artisan saher:record-gate \
  --verdict="$SAHER_VERDICT" --passed="$PASS" --failed="$FAIL" \
  ${SAHER_FAST:+$SAHER_FAST} 2>/dev/null || true
