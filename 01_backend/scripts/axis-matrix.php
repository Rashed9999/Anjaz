...
function extractAxes(string $doc, string $path): array
{
    ...
    foreach ($lines as $i => $line) {
        // **`)` و`.` كلتاهما تُرقّم محوراً.** كان المرشِّحُ يقبل `)` وحدَها،
        // فأخرج `t01` **صفرَ محاورٍ** من اثنين وخمسين — ولا رسالةَ خطأ:
        // وثيقةٌ كاملةٌ تختفي من التقرير بصمت، والمجموعُ يبدو سليماً.
        // وهو بعينه صنفُ العطل الذي بُنيت هذه الأداةُ لتكشفه، واقعاً عليها.
...
if (isset($opts['check'])) {
    if ($liars === []) {
        echo "  \033[32m✓\033[0m لا إغلاقَ بلا دليل (".count($report)." محوراً مفحوصاً)\n";
        exit(0);
    }
