...
class PdfSurfaceGuardTest extends TestCase
{
    ...
    private const EXEMPT = [
        ...
        // ملفُّ تسجيلٍ واحدٌ يفتحه مسؤولٌ من متصفّحِ مكتبٍ على شبكةٍ ثابتة،
        // ومحتواه يتغيّر مع كلّ تحديثٍ للملفّ — فمفتاحُ التخزين يُبطَل قبل
        // أن يُقرأ مرّتين. **والعلّةُ التي بُني لها الحارسُ غائبةٌ هنا**:
        // القطعُ على شبكة جوّالٍ في منتصف تصيير. والطولُ مُعلَنٌ فيُكشَف
        // القطعُ إن وقع.
        'Admin/RegistrationDossierController.php' => 'ملفُّ تسجيلٍ إداريٌّ من متصفّح، ومحتواه يتبدّل بكلّ تحديث',

        // ══════════════════════════════════════════════════════════════
        // **ولا يُخزَّن هذا أبداً، والسببُ أمنيٌّ لا أداء.**
        //
        // ورقةُ الحساب تُضمّن **صورَ الهويّة مفكوكةَ التشفير** داخلها
        // (AMIAL-ACCOUNT-PRINT-001). وطبقةُ التخزين تكتب الـPDF **خاماً
        // على القرص** في `storage/app/documents` — أي أنّ تمريرَها هنا
        // ينسخ كلَّ بطاقة هويّةٍ في المنصّة إلى ملفٍّ غيرِ مشفَّرٍ يبقى،
        // **فيُلغي بالضبط ما بُني `encrypted_path` كلُّه من أجله**.
        //
        // ولا يُصلحه مفتاحٌ أذكى ولا مهلةُ انتهاء: المشكلةُ أنّ النسخةَ
        // تُكتَب أصلاً. **وعلّةُ الحارس غائبةٌ هنا** — مسؤولٌ واحدٌ على
        // متصفّح مكتبٍ يطبع ورقةً بعد قرارٍ يتّخذه، لا عميلٌ على شبكة
        // جوّالٍ يفتح فاتورتَه عشرَ مرّات. والطولُ مُعلَنٌ فيُكشَف القطعُ
        // إن وقع.
        // ══════════════════════════════════════════════════════════════
        'Admin/AccountDossierPrintController.php' => 'يُضمّن صورَ هويّةٍ مفكوكةَ التشفير — والتخزينُ يكتبها خاماً على القرص',
    ];
