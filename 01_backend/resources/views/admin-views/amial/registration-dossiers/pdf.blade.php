...
@foreach($sections as $title => $fields)
  ...
  @if(count($visible))<h3>{{ $title }}</h3><table><tbody>@foreach(array_chunk($visible, 2) as $pair)<tr>@foreach($pair as $key)<th>{{ $labels[$key] ?? $key }}</th><td>{{ $key === 'is_pep' ? ((string)$payload[$key] === '1' ? 'نعم' : 'لا') : $payload[$key] }}</td>@endforeach @if(count($pair) === 1)<th></th><td></td>@endif</tr>@endforeach</tbody></table>@endif
@endforeach
