...
{{--
    **الـnonce إلزاميّ** — `SecurityHeaders` يضبط
    `script-src 'self' 'nonce-…'`، فسكربتٌ مضمَّنٌ بلا توقيعٍ **يرفضه
    المتصفّحُ صامتاً**: الصفحةُ تردّ ٢٠٠، والقائمةُ المنسدلةُ لا تُرشِّح،
    وحقلُ حصّة الوكيل لا يختفي، والمحاكي لا يعمل — **ولا خطأَ في أيّ سجلّ
    من جهة الخادم**.

    وقد وقع هذا فعلاً في أوّل صياغةٍ لهذه الشاشة، ولم يكشفه إلّا متصفّحٌ
    يفتح الصفحةَ **عبر الخادم**: فحصُ الضغط في البوّابة يبني القالبَ
    مباشرةً فلا يمرّ بالوسيط، فلا يرى المنعَ أبداً.
--}}
