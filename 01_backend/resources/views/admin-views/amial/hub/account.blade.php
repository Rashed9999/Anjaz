...
(function () {
    ...
    async function load() {
        ...
        if (u.merchant) {
            ...
            // ══════════════════════════════════════════════════════════
            // ══════════════════════════════════════════════════════════
            //  AMIAL-MERCHANT-360-DRILL-001 — **من الملفّ إلى مراكز الرقابة.**
            //
            //  ثلاثةُ مراكزَ عابرةٍ للتجّار (موظّفون · وقود · تجزئة) وفيها
            //  كلُّها تنقيبٌ لتاجرٍ بعينه — **ولا رابطَ منها ولا إليها من
            //  هنا.** فمن يحقّق في تاجرٍ كان يعود إلى القائمة الجانبيّة
            //  ويبحث عن اسمه من جديد في كلّ مركز.
            //
            //  وهذا ما جعل القوائمَ تبدو كثيرةً — لا عددُها. (القاعدة ١٢:
            //  المسارُ المسجَّل ليس ظهوراً، لا بدّ من رابطٍ يقود إليه.)
            // ══════════════════════════════════════════════════════════
            const drills = [
                ['👔 موظّفوه ونقاط بيعه', '{{ url('admin/amial/hub/staff') }}?merchant=' + id],
                ['🏪 مخزونُه وجردُه', '{{ url('admin/amial/retail') }}?merchant=' + id],
                ['⛽ محطّاتُه وفروقاتُها', '{{ url('admin/amial/fuel') }}?merchant=' + id],
            ];
            html += card('التحقيق في عمله',
                '<div class="d-flex flex-wrap gap-2">' + drills.map(([label, href]) =>
                    `<a class="btn btn-sm btn-outline-primary" href="${href}"
                        data-testid="drill-${href.split('/').slice(-1)[0].split('?')[0]}">${label}</a>`
                ).join('') + '</div>' +
                '<div class="form-text mt-2">مراكزُ الرقابة مفتوحةً على هذا التاجر وحده.</div>');
