...
{{--
    AMIAL-DEAD-STACK-001 — **مكدَّسٌ لا يعرضه أحد، وسكربتٌ بلا توقيع.**

    ══════════════════════════════════════════════════════════════════════
    عطلان في سطرين، وكلاهما يُنتج الأثرَ نفسَه: **صفحةٌ تردّ ٢٠٠ وسلوكُها
    ميّتٌ بالكامل**، ولا خطأَ في أيّ سجلّ من جهة الخادم.

    **① `@push('script_2')`** — والقالبُ الأمُّ يعرض `@stack('script')`
    وحدَه. فالشيفرةُ تُدفَع إلى مكدَّسٍ **لا يُصيَّر أبداً**، فلا تصل
    المتصفّحَ إطلاقاً.

    **② `<script>` بلا `nonce`** — و`SecurityHeaders` يضبط
    `script-src 'self' 'nonce-…'`، فيرفضه المتصفّحُ صامتاً حتّى لو وصل.

    وأثرُهما هنا بعينه ما يحذّر منه `CLAUDE.md`: **«صفحةٌ خضراءُ فوق راصدٍ
    ميّتٍ أسوأ من حمراء»** — قوائمُ تغيير حالة الأعطال معروضةٌ في مركز
    الأعطال ولا تُرسل طلباً واحداً. يُغيّر المشرفُ الحالةَ فلا يتغيّر شيء،
    ويظنّ العطلَ مُعالَجاً.
--}}
@push('script')
