...
class AccessConstants
{
    ...
    public const PLAN_LIMITS = [
        self::PLAN_FREE => [
            ...
            // ══════════════════════════════════════════════════════════
            // AMIAL-POS-SEAT-001 — **مقعدٌ يُباع ولا يُشغَّل.**
            //
            // كان هنا `employees => 0` مع `pos_devices => 1`. **وهما لا
            // يجتمعان**: الدخولُ إلى نقطة البيع يكون **برمز موظّف**
            // (`pos_users.pos_number`)، ولا موظّفَ إلّا بمقعدٍ مسموح.
            // فـ`assertCanAddEmployee` تقارن `0 >= 0` وترفض أوّلَ إنشاء.
            //
            // فالباقةُ تعِد بنقطة بيعٍ واحدة **ولا يمكن استعمالها أبداً** —
            // لا بترقيةٍ ولا بغيرها، لأنّ العطلَ في الجمع لا في العدد.
            // **وكشفه صاحبُ المشروع حين عجز عن تجربة نقطة البيع.**
            //
            // **والواحدُ هنا ليس هبةً بل تسليمُ ما بيع**: مقعدُ نقطة
            // البيع المُعلَن في الباقة يحتاج شاغلاً واحداً ليعمل. ومن
            // أراد ألّا تكون نقطةُ البيع في المجّانيّة أصلاً فليُنزل
            // `pos_devices` إلى صفر — **وذاك قرارُ تسعيرٍ يُتّخذ صراحةً**،
            // ولا يُترك تناقضاً صامتاً يُقرأ عطلاً.
            // ══════════════════════════════════════════════════════════
            'employees' => 1,
            'branches' => 0,
            'pos_devices' => 1,
