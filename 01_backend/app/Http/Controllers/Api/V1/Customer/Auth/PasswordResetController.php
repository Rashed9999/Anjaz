...
class PasswordResetController extends Controller
{
    ...
    public function resetPasswordRequest(Request $request): JsonResponse
    {
        ...
        if (isset($customer)) {
            ...
            // ══════════════════════════════════════════════════════════
            // AMIAL-OTP-ENV-001 — **استعادةُ كلمة المرور كانت مفتوحةً للجميع.**
            //
            // كان السطر: `(env('APP_MODE') != 'live') ? '1234' : rand(...)`.
            //
            // و`docker/entrypoint.prod.sh` يشغّل `php artisan config:cache`،
            // ولارافيل بعدها **لا يحمّل ملفّ `.env` إطلاقاً**. فـ`env()`
            // خارج ملفّات `config` تُرجع `null` في الإنتاج — و`null != 'live'`
            // صحيحة — **فالرمزُ كان `1234` دائماً على الخادم الحيّ**.
            //
            // ومعناه أنّ من يعرف رقمَ هاتف أيّ عميلٍ يستعيد كلمة مروره
            // بـ`1234`. وهو استيلاءٌ كاملٌ على الحساب، لا ثغرةُ راحة.
            //
            // **ولا سطرَ في أيّ سجلّ يدلّ عليه:** المتغيّر مضبوطٌ في
            // البيئة، والشيفرةُ تبدو صحيحةً لمن يقرؤها، والاختبارات تمرّ
            // لأنّها تعمل بلا إعدادٍ مخزَّن.
            //
            // فصار الرمزُ من `OtpPolicy` — البابِ الواحد: رقمُ العرض يأخذ
            // الثابت، وكلُّ رقمٍ حقيقيّ يأخذ عشوائيّاً **مهما كانت البيئة**.
