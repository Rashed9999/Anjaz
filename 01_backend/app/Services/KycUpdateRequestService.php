...
class KycUpdateRequestService
{
    ...
    public function request(User $customer, ?User $actor, string $source): array
    {
        return DB::transaction(function () use ($customer, $actor, $source) {
            ...
            // الخدمةَ يسقط بـ`ParseError`. أسقط ٢١ اختباراً وحدَه.
            //
            // **وتصحيحٌ لِما كُتب أوّلاً في هذا التعليق:** قلتُ إنّ طبقةَ
            // تركيب PHP في البوّابة «لم تمسكه». وقِيس فإذا هي **تمسكه**
            // (`find … -newermt '-14 days' | xargs php -l`) — والخطأُ كان
            // منّي: التقطتُ ذيلَ مخرجات البوّابة وحدَه فلم أرَ حكمَها،
            // فنسبتُ إليها عجزاً ليس فيها.
            //
