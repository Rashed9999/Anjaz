...
class RecipientVerificationService
{
    ...
    public function verifyRecipient(string $phone, int $senderId): array
    {
        ...
        // فحص أن المستلم في نفس المنطقة التشغيلية (SOUTH)
        if (($recipient->zone_code ?? 'UNKNOWN') !== 'SOUTH') {
            throw new RuntimeException('المستلم غير مؤهل لاستقبال التحويلات حالياً');
        }
