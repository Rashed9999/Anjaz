...
class RecipientVerificationService
{
    ...
    public function verifyRecipient(string $phone, int $senderId): array
    {
        ...
        // ══════════════════════════════════════════════════════════════
        // AMIAL-TRANSFER-KIND-001 — **نوعُ الحساب يُسأل هنا أيضاً.**
        //
        // كشفه صاحبُ المشروع بالاستعمال: حوّل من حساب عميلٍ إلى **رقم
        // حساب وكيل** فنجح. وأثرُه ليس هفوةً بل خللٌ في اقتصاد الشبكة:
        // فائضٌ وهميٌّ دائمٌ في التسوية، وسحبٌ نقديٌّ بلا سجلِّ سحب،
        // وفلوتٌ يدخل الوكيلَ مقابلَ لا شيء.
        //
        // **والمنعُ هنا وفي مسار المال معاً** (القاعدة الرابعة): هذه
        // تُصدر الرمزَ، وتلك تُحرّك المال — وإغلاقُ بابٍ وترْكُ الآخر
        // مفتوحاً ليس إغلاقاً.
        //
        // **واستُعيد بعد أن حُذف في دفعةٍ لاحقة**، وأمسكه
        // `PeerTransferRecipientKindGuardTest` بحالتيه.
        // ══════════════════════════════════════════════════════════════
        $this->assertReceivableKind($recipient);

        // ══════════════════════════════════════════════════════════════
        // فحص أهلية المستلم — **والرفضُ يقول ما الناقص.**
        //
        ...
        if ((int) ($recipient->is_kyc_verified ?? 0) !== 1) {
            ...
            // «لم يُعتمد» تُقرأ **رفضاً وقع** — فيظنّ المرسِلُ الحسابَ
            // محظوراً أو مشبوهاً؛ و«لم يُوثَّق بعد» تُقرأ **انتظاراً
            // يمرّ**. والفرقُ هو كلُّ ما يحتاجه القارئ، وقد وصل هذا
            // العطلُ عبر شاشة صاحب المشروع لا عبر جهاز رصد.
            //
            // وهي صيغةُ المشروع لهذه الحالة بعينها — يستعملها تعليقُ
            // هذه الدالّة أعلاه و`DataTruthCollector` كلاهما.
            throw new RuntimeException(
                'حساب المستلم لم يُوثَّق بعد — لا يستقبل تحويلات حتى يكتمل التوثيق: تُراجَع هويته ويُفعَّل حسابه.'
            );
        }
