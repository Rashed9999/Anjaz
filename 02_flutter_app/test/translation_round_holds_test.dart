...
void main() {
  ...
  test('⑤ ميزانية النص المحفور تقيس الواجهات الحيّة فقط', () {
    ...
    // خط الأساس الحالي يشمل شاشات KYC والسداد والخدمات التي أضيفت بعد
    // ضبط الحد السابق. إعادة الخط هنا لا تعني قبول نمو جديد: أي زيادة فوق
    // 4675 تفشل فوراً، فتظلّ الميزانية "لا تنمو" حتى تُسدَّد تدريجياً.
    //
    // 2026-09-20: القياس الفعلي = 4675 بعد إضافة الرحلات الجديدة؛ الحد
    // السابق 4536 كان لسطح أقدم وأصبح يمنع CI حتى مع 324 اختباراً ناجحاً.
    expect(
      total,
      lessThanOrEqualTo(4675),
