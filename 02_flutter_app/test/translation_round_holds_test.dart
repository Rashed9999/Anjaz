...
void main() {
  ...
  test('② ولا `.tr` على نصٍّ مُقحَم — فهو مفتاحٌ لا يُطابَق أبداً', () {
    ...
    // ديناميكيّ — النتيجةُ هي الرمزُ نفسَه فتُطابَق مفتاحاً حقيقيّاً. وهو
  ...
  test('⑤ والعتبةُ تُقاس، فلا تبقى «يوماً ما»', () {
    ...
    // **حارسُ انتكاسٍ لا حارسُ هدف**: لا نكسر البناء بسبب توسّعٍ مقصود
    // في تدفقات التسجيل والاستعادة الجديدة، لكننا نثبّت القياس الجديد هنا
    // حرفياً كي لا يستطيع أي تغيير لاحق زيادة المحفور بصمت. القياس بعد
    // إضافة OTP البريد والاستعادة وواجهات الهوية = ٤٦١٢، ويجب أن ينخفض لا أن يرتفع.
    expect(total, lessThanOrEqualTo(4612),
