...
class _MerchantPayScreenState extends State<MerchantPayScreen> {
  ...
  Future<void> _confirmAndPay(Map<String, dynamic> data) async {
    ...
    // **ورمزُ الطلب يُقرأ من الردّ لا من مُعامِلٍ لم يعد موجوداً.**
    //
    // ══════════════════════════════════════════════════════════════════
    // العطلُ الذي وُلد منه هذا السطر: لمّا وُحّد الطريقان — المسحُ وإدخالُ
    // رقم الفاتورة — في هذه الدالّة، بقي النداءُ `pr.pay(code, …)` يشير
    // إلى مُعامِل الدالّة القديمة `_payFixedRequest(String code)`. فسقط
    // البناءُ كلُّه على `Undefined name 'code'`.
    //
    // والخادمُ يطابق بـ`short_code` (`PaymentRequestService::findByCode`)،
    // وكلا الطريقين يُرجع `request.short_code` — فهو المعرّفُ الوحيد الذي
    // يصلح للطريقين معاً. (القاعدة الرابعة: ميزةٌ لها مدخلان تُختبر من
    // مدخليها — ودمجُهما يجب ألّا يُسقط ما يحتاجه أحدُهما.)
    final code = '${req['short_code'] ?? ''}';
    ...
    // **ولا تُعرض نافذةُ تأكيدٍ لدفعةٍ لا يمكن تنفيذها.**
    // فردٌّ ناقصُ `short_code` يعني عطلاً في الخادم أو في الشكل، ومن
    // أكّد ثمّ أدخل رمزَ حمايته ثمّ فشل يظنّ أنّ المالَ خرج.
    if (code.isEmpty) {
      _err('تعذّر قراءة رمز الطلب — أعد المسح أو تحقّق من رقم الفاتورة');
      return;
    }
