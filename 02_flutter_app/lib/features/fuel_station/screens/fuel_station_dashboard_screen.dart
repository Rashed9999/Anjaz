...
class FuelStationDashboardScreen extends StatefulWidget {
  ...
  @override
  State<FuelStationDashboardScreen> createState() =>
      _FuelStationDashboardScreenState();
}

class _FuelStationDashboardScreenState
    extends State<FuelStationDashboardScreen> {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                ...
                return Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (d?['by_payment'] != null &&
                          (d!['by_payment'] as List).isNotEmpty) ...[
                        const SizedBox(height: 12),
                        const Divider(),
                ...
                children: [
                  ...
                  Expanded(
                    child: _miniAction(
                      Icons.business,
                      'الشركات',
                      () => Get.to(() => const FuelCompaniesScreen()),
                    ),
                  ),
                ],
              ),
                ...
                children: [
                  Expanded(
                    child: _miniAction(
                      Icons.history,
                      'سجل المبيعات',
                      () => Get.to(() => const FuelSalesHistoryScreen()),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: _miniAction(
                      Icons.tune,
                      'الإعدادات',
                      () => Get.to(() => const FuelSettingsScreen()),
                    ),
                  ),
                ],
              ),
                ...
                children: [
                  Expanded(
                    child: _miniAction(
                      Icons.receipt_long,
                      'إعدادات الفاتورة',
                      () => Get.to(() => const ReceiptSettingsScreen()),
                    ),
                  ),
                  const SizedBox(width: 8),
              ...
              // AMIAL-FUEL-DASH-001 — اللوحة لمحطة الوقود، والباقي في «خدماتي».
              //
              // كانت تعرض خمس عشرة بلاطة ميزة تحت عنوان «حسب باقتك»، فصارت
              // قائمةً طويلة يمرّ عليها صاحب المحطة كل صباح ليصل إلى الكاشير.
              // وأكثرها لا تخصّ محطة وقود أصلاً (وصفات صيدلية، تسعير جملة).
              //
              // فبقي هنا ما تحتاجه محطة يومياً، ونزل الباقي إلى شاشة «خدماتي»
              // حيث يُرتَّب بالملكية: المفتوح أوّلاً والمقفل أسفله.
              _myServicesCard(),
            ],
          ),
        ),
  ...
  Widget _fuelHourly(Map d) {
    final byHour = (d['by_hour'] as List)
  ...
  Widget _currentShiftCard() {
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'لا توجد مناوبة مفتوحة — افتح مناوبة لبدء العمل',
    ...
    final todayCash =
        double.tryParse(
          '${(today?['by_payment'] as List?)?.firstWhere((m) => (m as Map)['payment_method'] == 'cash', orElse: () => {'total': 0})['total'] ?? 0}',
        ) ??
        0;
      ...
      return InkWell(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Icon(
                isFree ? Icons.lock_open_outlined : Icons.workspace_premium,
                size: 13,
                color: isFree ? Colors.white : Colors.black87,
              ),
              const SizedBox(width: 4),
              Text(
                'باقتك: $label${isFree ? ' • ترقية' : ''}',
                style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.bold,
                  color: isFree ? Colors.white : Colors.black87,
