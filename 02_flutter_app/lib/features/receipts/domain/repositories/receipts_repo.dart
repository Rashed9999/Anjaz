...
class ReceiptsRepo extends GetxService {
  ...
  Future<Response> list({String? type, int page = 1}) async {
    ...
    return apiClient.getData('${AppConstants.amyalReceiptsList}?$query');
  }

  Future<Response> show(int id) async {
    return apiClient.getData('${AppConstants.amyalReceiptShow}$id');
  }

  /// نُرجع الـ URL للتحميل المباشر (الـ Flutter يستخدم url_launcher / file save).
  String downloadUrl(int id) {
    return '${apiClient.appBaseUrl}${AppConstants.amyalReceiptDownload}$id/download';
  }

  Future<Response> verifyPublic(String code) async {
    return apiClient.getData('${AppConstants.amyalReceiptVerifyPublic}$code');
  }
}
