...
/// 1) أربع معلومات أساسية + موافقة الشروط.
...
class _QuickRegistrationScreenState extends State<QuickRegistrationScreen> {
  ...
  bool _validateBasics() {
    ...
    if (!RegExp(r'^\d{4}$').hasMatch(_pin.text)) {
      _snack('رمز PIN يجب أن يتكون من 4 أرقام.');
      return false;
    }
  ...
  Future<void> _requestPhoneOtp({bool silentSuccess = false}) async {
    ...
    try {
      ...
      final ok = response.statusCode == 200 &&
          ...
          response.body['message']?.toString() == 'success';
      if (!ok) {
        _snack(
          _responseMessage(
            response,
      ...
      if (!silentSuccess) {
        _snack('أرسلنا رمز التحقق إلى رقم هاتفك.', error: false);
      }
    } catch (_) {
  ...
  Widget _basicsStep() => _page(children: [
        _card([
          _title(
            'حسابك الأساسي',
            'نحتاج فقط بيانات الاتصال وPIN الآن. يمكنك توثيق الهوية ورفع الحدود لاحقاً من حسابك.',
