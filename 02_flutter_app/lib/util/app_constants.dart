...
class AppConstants {
  ...
  /// AMIAL-I18N-003 — **زرٌّ يُضغط ولا يتغيّر شيء.**
  ///
  /// ══════════════════════════════════════════════════════════════════
  /// قال صاحبُ المشروع: «زرُّ الترجمة لا يعمل — العربيّةُ موجودة، **لكن
  /// عند تغيير الإنجليزيّة لا يتغيّر شيء**».
  ///
  /// **وتُتبِّعت السلسلةُ كاملةً فإذا هي سليمة:** الزرُّ ←
  /// `LocalizationController.setLanguage` ← `Get.updateLocale` ←
  /// `Messages` ← `assets/language/{ar,en}.json` (٤٤٨ مفتاحاً لكلٍّ).
  ///
  /// **والعطلُ في التغطية لا في الآليّة.** الترجمةُ تُطبَّق على ما يُكتب
  /// `'key'.tr` وحدَه، وأسوأُ من النسبة توزيعُها: الشاشاتُ التي يقف عليها
  /// التاجر كانت الأقلَّ ترجمة — لوحةُ التجزئة **صفرُ نداءات** مقابل ٤١
  /// نصّاً، و«خدمات التاجر» ١ مقابل ١٢٨، والباقات ٦ مقابل ٦١.
  ///
  /// ─────────────────────────────────────────────────────────────────
  /// **AMIAL-I18N-004 — وجولةٌ مخصَّصةٌ جرت، وهذا ما بلغته بالقياس:**
  ///
  ///     الشاشاتُ الخمسُ الرئيسيّة  →  **صفرُ نصٍّ محفورٍ فيها كلِّها**
  ///     مفاتيحُ اللغة            →  ٤٤٨ ← **٧٤٧** (٢٩٩ مفتاحاً جديداً)
  ///     `TextDirection.rtl` مفروضاً →  ١٣ ← **صفر** (تقرأ `isLtr` الآن)
  ///
  /// **ولم تُعَد الإنجليزيّةُ مع ذلك — والسببُ رقمٌ لا رأي:**
  ///
  ///     نداءاتُ `.tr` في التطبيق  :   ٧٦٦
  ///     نصوصٌ عربيّةٌ محفورةٌ باقية :  ٤٥٣٤  في ٢٤٤ ملفّاً
  ///
  /// أي **١٤٪** تغطيةً. ومعظمُ الباقي أبوابٌ يمرّ بها كلُّ مستعمل: معالجُ
  /// التسجيل (١٨٢) · شاشةُ الدخول (٥٨) · لوحةُ الزبون (٦١) · الجملةُ
  /// بشاشاتها الثلاث (٣٢١) · التقارير (٧٣). **فمن بدّل اليومَ يرى خمسَ
  /// شاشاتٍ تتغيّر وبقيّةَ التطبيق عربيّاً** — وهو العطلُ المُبلَّغ نفسُه،
  /// أصغرَ قليلاً.
  ///
  /// **ووعدٌ لا يُوفى أسوأ من غيابه** (القاعدة التاسعة). والعتبةُ تُكتب
  /// هنا صريحةً لئلّا تبقى «يوماً ما»: **تُعاد يومَ يبلغ المحفورُ الباقي
  /// دون ٥٠٠ نصٍّ**، أي حين تُترجَم أبوابُ الدخول والتسجيل والزبون
  /// والجملة والتقارير. ولا تُعاد قبلَه.
  ///
  /// **ولا يُهدَم ما بُني**: `en.json` مطابقٌ لـ`ar.json` مفتاحاً بمفتاح،
  /// و`Messages` و`LocalizationController` كما هي — فالعودةُ سطرٌ واحد.
  /// وصفُّ «تغيير اللغة» ورقاقتُها محجوبان بـ`length > 1`، فيختفيان من
  /// تلقائهما ويعودان كذلك.
  ///
  /// يظهر في : التطبيق ← الملفّ الشخصيّ (يختفي صفُّ اللغة) · وشاشةُ
  /// الدخول (تختفي رقاقةُ اللغة). وفي لوحة الإدارة: لا.
  static List<LanguageModel> languages = [
    ...
    // **تُعاد بنزع التعليق** يومَ يهبط المحفورُ الباقي دون ٥٠٠ — والعدُّ
    // محروسٌ في `no_english_reaches_the_eye_test` فيُقال كم بقي.
