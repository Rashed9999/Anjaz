...
class AppConstants {
  ...
  /// `'key'.tr` وحدَه، والمقيسُ في التطبيق:
  ///
  ///     نداءاتُ `.tr`                :  359
  ///     نصوصٌ عربيّةٌ محفورةٌ في الشيفرة :  825
  ///
  /// **وأسوأُ من النسبة توزيعُها** — الشاشاتُ التي يقف عليها التاجر هي
  /// الأقلُّ ترجمة: لوحةُ التجزئة **صفرُ نداءات** مقابل ٤١ نصّاً محفوراً،
  /// و«خدمات التاجر» ١ مقابل ١٢٨، والباقات ٦ مقابل ٦١. فمن بدّل إلى
  /// الإنجليزيّة وهو على لوحته **لا يرى محرفاً واحداً يتغيّر**.
  ///
  /// وثالثةٌ: `TextDirection.rtl` مفروضٌ في **١٣ موضعاً**، فحتّى المترجَمُ
  /// يبقى تخطيطُه من اليمين.
  ///
  /// **فرُفعت الإنجليزيّةُ من القائمة، ولم تُحذَف من المشروع**: ملفُّ
  /// `en.json` باقٍ بمفاتيحه، و`Messages` و`LocalizationController` كما
  /// هي — فإعادتُها سطرٌ واحدٌ يومَ تُترجَم الشاشاتُ فعلاً.
  ///
  /// **ووعدٌ لا يُوفى أسوأ من غيابه** (القاعدة التاسعة: زرٌّ يُضغط ولا
  /// يحدث شيءٌ يُقرأ عطلاً، ويُرسل صاحبَه يبحث عمّا لا وجودَ له). وصفُّ
  /// «تغيير اللغة» في الملفّ الشخصيّ محجوبٌ سلفاً بـ`length > 1`، فيختفي
  /// من تلقائه.
  ///
  ...
  static List<LanguageModel> languages = [
    ...
    // **تُعاد بنزع التعليق** يومَ تُترجَم شاشاتُ التاجر — ولا تُعاد قبله.
