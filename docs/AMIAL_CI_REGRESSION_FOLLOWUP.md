...
| `01_backend/tests/Feature/NoTillWithoutAShiftGuardTest.php` | Freeze the daytime-hours scenario at 14:00 so both shifts occur today even when CI executes after midnight; preserve its five-hour expectation. |
...
Commit `f75d4e9d80a90c18150a0f13d36b9b4c301f196e`, [run 34791587022](https://github.com/Rashed9999/Anjaz/actions/runs/34791587022): the expanded email gate passed **95 tests / 432 assertions**, and the settlement/prior-blocker gate passed **140 tests / 468 assertions**. Structural, Flutter and both Docker jobs passed. Full backend: **3,916 passed, 1 failed, 17 risky, 9 skipped, 52,829 assertions**, in 818.35 seconds. The remaining shift-hours fixture created an 08:00–11:00 shift and a running shift relative to the real clock; after midnight its assumed five hours were not all inside today. Freeze this specific daytime scenario at 14:00; do not change production time-window calculations. Provider-readiness findings and the additional simulator guard are tracked in `AMIAL_BILL_PROVIDER_READINESS.md`.
